...
describe("BAV CRI happy path tests", () => {
	...
	describe("/userinfo Endpoint", () => {
		...
		it("Successful Request Test", async () => {
			...
			const experianUuid = await getKeyFromSession(sessionId, constants.DEV_BAV_SESSION_TABLE_NAME, "experianUuid");
			expect(decodedBody.vc.evidence[0].txn).toBe(experianUuid);

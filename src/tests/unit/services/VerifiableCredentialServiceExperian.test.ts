...
import { VerifiableCredentialServiceExperian, VerifiableCredentialBuilder } from "../../../services/VerifiableCredentialServiceExperian";
...
describe("VerifiableCredentialService", () => {
	...
	let service: VerifiableCredentialServiceExperian;

	beforeEach(() => {
		jest.clearAllMocks();
		service = new VerifiableCredentialServiceExperian( mockKmsJwtAdapter, mockIssuer, mockLogger, dnsSuffix);
	});

	describe("getInstance", () => {
		it("should create a new instance if not already created", () => {
			const newInstance = VerifiableCredentialServiceExperian.getInstance( mockKmsJwtAdapter, mockIssuer, mockLogger, dnsSuffix);
			expect(newInstance).toBeDefined();
		});

		it("should return the same instance of VerifiableCredentialService when called multiple times", () => {
			const firstInstance = VerifiableCredentialServiceExperian.getInstance( mockKmsJwtAdapter, mockIssuer, mockLogger, dnsSuffix);
			const secondInstance = VerifiableCredentialServiceExperian.getInstance( mockKmsJwtAdapter, mockIssuer, mockLogger, dnsSuffix);

...
describe("JwksHandler", () => {
	describe("#handler", () => {
		beforeEach(() => {
			process.env.SIGNING_KEY_IDS = "bav-cri-api-vc-signing-key";
			process.env.ENCRYPTION_KEY_IDS = "bav-cri-api-encryption-key";
		...
		it("uploads keys to s3", async () => {
			...
			expect(logger.info).toHaveBeenCalledWith({ message:"Building wellknown JWK endpoint with keys" + ["bav-cri-api-vc-signing-key", "bav-cri-api-encryption-key"] });
			expect(handlerClass.s3Client.send).toHaveBeenCalledWith({
				Bucket: "bav-cri-api-jwks-dev",
	...
	describe("#getAsJwk", () => {
		it("gets the kms key with the given KeyId and returns jwk with public key", async () => {
			const keyId = "bav-cri-api-vc-signing-key";

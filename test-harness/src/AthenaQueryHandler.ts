...
const POWERTOOLS_LOG_LEVEL =
  process.env.POWERTOOLS_LOG_LEVEL ?? Constants.DEBUG;
const POWERTOOLS_SERVICE_NAME =
  process.env.POWERTOOLS_SERVICE_NAME ?? Constants.DEQUEUE_LOGGER_SVC_NAME;
...
class AthenaQueryHandler implements LambdaInterface {
	...
	async handler(
      ...
      (process.env["GLUE_TABLE"] ?? "") +
      '" WHERE timestamp >= ? AND cicname LIKE ? ORDER BY timestamp DESC';
		...
		const startQueryExecutionCommandOutput = await athenaClient.send(
			new StartQueryExecutionCommand(queryExecutionInput)
    );
		logger.debug("Query start response", {
		...
		do {
			...
			getQueryExecutionCommandOutput = await athenaClient.send(
				getQueryExecutionCommand,
			);
			logger.debug("Get Query output", {
      ...
      queryState === QueryExecutionState.RUNNING
		);
		...
		if (queryState === QueryExecutionState.SUCCEEDED) {
			...
			const getQueryResultsCommandOutput = await athenaClient.send(
				getQueryResultsCommand,
			);
			logger.info("GetQueryResultsCommandOutput : ", {
				output: getQueryResultsCommandOutput.ResultSet?.Rows,
			});
			...
			logger.error(
				`Query failed: ${getQueryExecutionCommandOutput.QueryExecution?.Status?.StateChangeReason}`
      );
			return {
				statusCode: 500,
				body: "ERROR",
